let slideIndex = 0;
    ...
    const visibleItems = 3; // Mostrar 3 imágenes a la vez

    function moveCarousel2(direction) {
        currentIndex2 += direction * visibleItems;

        // Evitar que el índice sea menor que 0 o mayor que la cantidad total de imágenes
        if (currentIndex2 < 0) {
            currentIndex2 = totalItems2 - visibleItems;
        } else if (currentIndex2 >= totalItems2) {
            currentIndex2 = 0;
        }

        // Mover el carrusel
        const offset = -currentIndex2 * (100 / visibleItems); // Calcular el offset en base a las imágenes visibles
